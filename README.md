...
- **`Navbar`** — `{ title: string; shortTitle?: string; links: NavLink[]; brand?: React.ReactNode; prodApex: string; centerLinksOnPage?: boolean }`. Renders the header, mobile drawer, and `ThemeToggle`. If `shortTitle` is given, the header shows the full `title` on mobile (where links are hidden) and at `lg`+ (where there's room again), swapping to `shortTitle` only in the narrower `md`-to-`lg` band where the title is competing with the centered links for space. By default, links are centered in the space between the title and controls (never overlapping either, even if the title or link list is long). Set `centerLinksOnPage` to instead center links on the full header width, so they line up with page content that's centered on the page below — best suited to a short link list where overlap isn't a realistic risk.
